...
# passing from input
da<-read.csv("data/our_2DlowExpr/our_2DlowExpr.csv")
dir_sims = 'result/nnSVG/our_2DlowExpr.csv'
X <- da[, c("x", "y")]
...
write.table(rowData(spe1)['padj'],file=dir_sims,quote=FALSE,sep=",",col.names=FALSE)
## debug
end_time <- Sys.time()
print(mem_used())
