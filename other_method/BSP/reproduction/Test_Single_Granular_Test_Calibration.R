...
InputPath <- "../Data\\Single_Granular_Test\\P_value\\"
InputPath_Default <- "../Data\\Diff_Granular\\"
InputFiles <- list.files(InputPath)
OutputPath <- "../Data\\Single_Granular_Test\\Calibrated_P_value\\"
for(InputFile in InputFiles){
  InputData <- read.csv(paste0(InputPath, InputFile))
  OutputData <- apply(InputData[,-1], 1, metap::sumz)
  OutputData <- sapply(1:length(OutputData), function(i) return(as.numeric(OutputData[[i]]$p)))
  InputData_Default <- read.csv(paste0(InputPath_Default, InputFile))
  OutputData <- data.frame(genes = InputData$genes,
                           Calibrated = OutputData,
                           Default = InputData_Default$X3.0)
  write.csv(OutputData, row.names = FALSE,
            paste0(OutputPath, InputFile))
}

InputData <- read.csv(paste0(OutputPath, list.files(OutputPath)[1]))
InputData$type <- c(rep("SVG", 1000),
                    rep("Non-SVG", 9000))
library(ggplot2)
png(file="../Outputs/Calibrated_Pvalue_Hist.png",
    width=5, height=3, units = "in", res = 600)
ggplot(InputData, aes(x=Calibrated, fill=type)) +
  geom_histogram( color="#e9ecef", alpha=0.6, position = 'identity') +
  scale_fill_manual(values=c("#69b3a2", "#404080")) +
  labs(fill="") +
  xlab("Calibrated P-values")
dev.off()
...
InputPath <- c("../Data\\Single_Granular_Test\\Calibrated_P_value\\")


Method_Labels <- data.frame(Raw = c("Calibrated", "Default"),
                            Labels = c("Calibrated_Pvalue", "Default"))
png(file="../Outputs/Calibrated_Pvalue.png",
