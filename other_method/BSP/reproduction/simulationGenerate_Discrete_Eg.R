...
sim_discrete <- function(InputExp, OutputDir,
  ...
  for(i in 1:numSlices){
    Coords <- spatstat.random::rpoispp(N, 
                                       win = spatstat.geom::owin(c(0, 1), 
                                                                 c(0, 1)))
    SimData <- rbind(SimData, data.frame(x = Coords$x * (sqrt(N) - 1), 
                                         y = Coords$y * (sqrt(N) - 1), 
                                         z = i))
  }
  N_Obs <- nrow(SimData)
  ...
  spikedIndex <- data.frame(Indx = SpikedCellIndex)
...
# noise 1
for(i in 1){
  sim_discrete(InputExp, OutputDir,
               NGenes = 1000,
               NX = 30, NY = 30, numSlices = 10,
               Rad = 2, Breaks = 8,
               Quant = 0.80, Noise_lv = 0,
               SEED = i)
}