...
library(gridExtra)
library(ggpubr)
InputDataPath <- ".\\Data\\Manuscript_3D_Simulation_Power\\Out_1.0_3.0"
...
InputDataFiles <- list.files(paste0(InputDataPath, InputFolders[1]))
InputPath <- paste0(InputDataPath, InputFolders[1], "\\")
InputFiles <- InputDataFiles
InputFiles <- InputFiles[which(grepl("1_mergedPvalue", InputFiles))]
InputFiles <- InputFiles[c(3, 6, 9)]
...
for(i in 1:length(InputFiles)){
  ...
  PowerMatrixAll$Methods <- factor(PowerMatrixAll$Methods, levels = c("bsp_pvalue",
                                                                      "sparkx_pvalue"))
  levels(PowerMatrixAll$Methods) <- c("BSP", "SPARK-X")
  Col_plate <- c("#E64B35FF", "#4DBBD5FF", "#00A087FF", "#3C5488FF", 
                 "#F39B7FFF", "#8491B4FF", "#7E6148FF", "#DC0000FF")
...
png(file=".\\Outputs\\Manuscript_3D_Simulation_Power.png",
