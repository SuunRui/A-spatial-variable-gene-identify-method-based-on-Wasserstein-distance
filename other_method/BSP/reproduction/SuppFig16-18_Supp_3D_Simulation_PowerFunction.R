################################################################
# Function for Power analysis in 3D simulations
################################################################
...
powerplot <- function(InputPath){
  ...
  if(grepl("Scenario1", InputPath, ignore.case = TRUE)){
    InputFiles <- InputFiles[as.numeric(outer(c(1,3,2),(0:2)*3,"+"))]
  }
  if(grepl("Scenario3", InputPath, ignore.case = TRUE)){
    InputFiles <- InputFiles[as.numeric(outer(c(3,2,1),(0:2)*3,"+"))]
  }
  ggplot_list <- list()
