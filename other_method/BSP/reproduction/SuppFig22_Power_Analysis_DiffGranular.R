#############################################################
# Power analysis for different granularities
...
library(ggplot2)
library(ggpubr)
InputPath <- c("../Data/Diff_Granular/")

Method_Labels <- data.frame(Raw = c("X2.0", "X2.5", "X3.0", "X3.5", "X4.0", "X5.0", "X10.0"),
                            Labels = c("2.0", c("2.5", "3.0", "3.5", "4.0", "5.0", "10.0")))
png(file="../Outputs/Diff_Granular.png",
    width=9, height=3, units = "in", res = 600)
powerplot(InputPath = InputPath, Method_Labels = Method_Labels, Legend_title = "D2", Ncol= 3, Nrow = 1)
dev.off()
