...
library(pryr)
# library(reticulate)
# use_python("D:/annconda/envs/spatialDE/python.exe")
gc()
args = commandArgs(trailingOnly=TRUE)
da<-read.csv("data/our_2DfullExpr/our_2DfullExpr.csv")
# passing from input
# da<-read.csv(args[1])
X <- da[, c("x", "y")]
...
resid_expr <- regress_out(expr, sample_info = da_info)
...
write.csv(results, 'result/spatialDE/our_2DfullExpr.txt', row.names = FALSE, col.names = TRUE)
